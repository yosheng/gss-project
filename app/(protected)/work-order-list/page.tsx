...
export default function WorkOrderListPage() {
    ...
    try {
      ...
      const data = await GssApiService.fetchWorkOrderList({
        ...
        arg: {
          ...
          actNo: null, empNo: null, caseContNo: null, isPrdOrPjt: null,
          prdPjtNo: null, ttlHours: null, actTypeId: null, isPrnToCust: null,
          description: null, log: null, isAttachFile: null, place: null,
          attachFileName: null, nextActionDesc: null, custNo: null
        }
      });
      setWorkOrders(data);
    } catch (err) {
  ...
  useEffect(() => {
    if (dateFilter.startDate && dateFilter.endDate) fetchWorkOrders();
          ...
          <div className="flex flex-col space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <Input type="date" value={dateFilter.startDate} onChange={(e) => handleDateFilterChange('startDate', e.target.value)} className="min-h-[44px] touch-manipulation" />
              </div>
              <div className="flex items-center gap-2">
                <span className="text-gray-400">至</span>
                <Input type="date" value={dateFilter.endDate} onChange={(e) => handleDateFilterChange('endDate', e.target.value)} className="min-h-[44px] touch-manipulation" />
              </div>
            </div>
          </div>
        </CardHeader>
        <CardContent>
          {error ? (
            <div className="text-center py-12">
              ...
              <Button onClick={fetchWorkOrders} variant="outline">重試</Button>
            </div>
          ) : filteredWorkOrders.length === 0 ? (
            <div className="text-center py-12">
              <FontAwesomeIcon icon={faSearch} className="text-gray-300 text-4xl mb-4" />
              <p className="text-gray-500 font-medium">沒有找到符合條件的工作單</p>
              <p className="text-gray-400 text-sm mt-1">請調整搜尋條件或日期範圍</p>
            </div>
          ) : (
            <div className="space-y-4">
              {filteredWorkOrders.map((workOrder) => (
                <div key={workOrder.actNo} className="bg-white border rounded-lg p-4 sm:p-6 hover:bg-blue-50 transition-colors employee-card">
                  <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-col sm:flex-row sm:items-center gap-2 mb-3">
                        <h3 className="font-semibold text-blue-600 text-lg">{workOrder.actNo}</h3>
                        <span className="text-sm text-gray-500 bg-gray-100 px-2 py-1 rounded">{workOrder.actTypeName}</span>
                      </div>
                      <div className="space-y-2 text-sm">
                        <div>
                          <span className="font-medium text-gray-700">工作摘要:</span>
                          <p className="text-gray-900 mt-1 leading-relaxed">{truncateText(workOrder.description, 200)}</p>
                        </div>
                        ...
                        {workOrder.prdPjtNo && (
                          <div>
                            ...
                            <p className="text-gray-900 mt-1">{truncateText(workOrder.prdPjtNo, 150)}</p>
                          </div>
                        )}
                      </div>
                    </div>
                    <div className="lg:w-64 flex-shrink-0">
                      <div className="bg-gray-50 rounded-lg p-3 space-y-2 text-sm">
                        <div>
                          ...
                          <p className="text-gray-900">{formatDate(workOrder.sdateTime)}</p>
                        </div>
                        <div>
                          <span className="font-medium text-gray-700">結束時間:</span>
                          <p className="text-gray-900">{formatDate(workOrder.edateTime)}</p>
                        </div>
                        <div>
                          <span className="font-medium text-gray-700">總時數:</span>
                          <span className="text-blue-600 font-semibold ml-2">{formatDuration(workOrder.ttlHours)}</span>
                        </div>
                      </div>
                    </div>
                  </div>
